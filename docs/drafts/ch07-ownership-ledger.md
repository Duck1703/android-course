...
| 5 | `#quyen` — DittoSyncPermissions, manifest merger, không có `onRequestPermissionsResult`/`refreshPermissions` | **AP2** | HOW (helper API của SDK) — toàn bộ manifest-merger và refreshPermissions sống ở AP2 mục 3; quyết định khi dựng: O1 thuần kiến trúc, không rải chi tiết SDK. |
...
| 17 | `#init-chain` — init → initDatabase → postInitActions; KDoc sai; `GlobalScope` critique; race được cứu nhờ StateFlow | **AP3** (Ch07 row 7 — GlobalScope critique) + **O1** (race concept, thu gọn trong mục 5) | Critical reading đúng job O1; GlobalScope là điểm giáo-trình-nói-vs-code-thật → bảng AP3 theo chính sách mục 28. Chi tiết số dòng bỏ. |
...
| 19 | `#ghi-du-lieu` — UTC/ISO-8601 sort-key; map-of-doc; không ghi `_id`; `upsert` = update+insert trả về ngay (local-first) | **O1** (ý ghi trả về ngay, không chờ mesh) + **AP2** (map-of-doc, `_id` vứt — mục 4/5) + **REMOVE-with-reason**: phân tích chuỗi ISO-8601 làm sort-key (toIso8601String cắt chuỗi gắn Z) không tái dựng — chi tiết Kotlin/string của project cũ, ngoài job O1 (concepts) và AP2 (API reference); giá trị string-sort-key giữ ở dòng sort/ORDER BY của AP2 mục 4. |
...
| 22 | `#viewmodel` — starter (giữ list trong RAM) vs final (ViewModel lắp flow); `combine` = JOIN reactive; init shadowing bug | **REMOVE-with-reason** (S4 + W1) + **AP3** (Ch07 row 3 — shadowing bug) | ViewModel/StateFlow/combine đã dạy S4/W1; O1 chỉ cross-ref 1 câu. Shadowing bug sống trong bảng 15 điểm của AP3 (đúng chính sách mục 28). |
...
| 27 | `#dead-code` — addSubscriptionForRoom / addPrivateRoomSubscriptions / messagesDocs / authorImage | **O1** `#doc-code-cu-co-phe-phan` (đoạn phiên bản cắt của app lớn) + **AP3** (Ch07 row 8 — liệt kê đầy đủ) | Concept project giáo trình = bản cắt của app lớn, để lại vết — đúng đọc phê phán; danh sách chi tiết sống trong bảng AP3. |
